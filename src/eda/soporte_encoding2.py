...
def detectar_orden_cat(df, lista_cat, var_respuesta):
    ...
    try:
        for categorica in lista_cat:
            print(f"Evaluando variable: {categorica.upper()}")
            ...
            if p < 0.05:
                print(f"La variable {categorica} TIENE orden. (relación con la VR) ✅ [{round(p, 4)}]")
                var_ordenadas.append(categorica)
            else:
                print(f"La variable {categorica} NO TIENE orden (sin relación con la VR) ❌ [{round(p, 4)}]")
                var_noordenadas.append(categorica)
                
            print("___" * line_length + " \n")
